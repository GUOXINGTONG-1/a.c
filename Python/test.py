...
def test_SendText():
    ...
    chatwith = wx.GetFriendByWxNumber("sunday145237")
    print(chatwith)
...
def test_Message():
    wx = WeChatRobot()
    wx.StartService()
    wx.StartReceiveMessage(CallBackFunc = test_MessageCallBack)
    try:
        while True:
            time.sleep(1)
    except KeyboardInterrupt:
...
def test_MessageCallBack(robot,message):
    print(message)
